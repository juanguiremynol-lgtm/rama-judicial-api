...
// ✅ Fix: Selector correcto para contador de actuaciones
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.log(
          `[scraping ${jobId}] ℹ️ No se pudo leer contador de sujetos`
        );
      }
    ...
    try {
      ...
      // ========== FIX: Selector correcto para contador ==========
      try {
        const textoResultados = await page
          .locator('span.text--secondary:has-text("Resultados encontrados")')
          .first()
        ...
        console.log(`[scraping ${jobId}] ℹ️ No se pudo leer contador de resultados: ${error.message}`);
      }
...
const server = app.listen(PORT, () => {
  console.log(`🚀 API Rama Judicial v4.2 activa en puerto ${PORT}`);
